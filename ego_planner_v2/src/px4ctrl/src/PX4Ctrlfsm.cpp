...
PX4CtrlFSM::PX4CtrlFSM(Parameter_t &param_, LinearControl &controller_)
    : param(param_), controller(controller_) /*, thrust_curve(thrust_curve_)*/
{
  state = MANUAL_CTRL;
  hover_pose.setZero();
  rc_dy_data.reset();
...
void PX4CtrlFSM::process() {
  ...
  case MANUAL_CTRL: {
    if (rc_dy_data.enter_hover_mode) // Try to jump to AUTO_HOVER
    {
      ...
      if (cmd_is_received(now_time)) {
        ...
        ROS_ERROR("[px4ctrl] Reject AUTO_HOVER(L2). You are sending commands "
                  "before toggling into AUTO_HOVER, which is not allowed. Stop "
                  "sending commands now!");
        break;
      }
      ...
      ROS_INFO("\033[32m[px4ctrl] MANUAL_CTRL(L1) --> AUTO_HOVER(L2)\033[32m");
               ...
               takeoff_land_data.takeoff_land_cmd ==
                   quadrotor_msgs::TakeoffLand::TAKEOFF) { // Try to jump to
                                                           // AUTO_TAKEOFF
      ...
      if (cmd_is_received(now_time)) {
        ROS_ERROR("[px4ctrl] Reject AUTO_TAKEOFF. You are sending commands "
                  "before toggling into AUTO_TAKEOFF, which is not allowed. "
                  "Stop sending commands now!");
        break;
      }
      if (odom_data.v.norm() > 0.1) {
        ROS_ERROR("[px4ctrl] Reject AUTO_TAKEOFF. Odom_Vel=%fm/s, non-static "
                  "takeoff is not allowed!",
                  odom_data.v.norm());
        break;
      }
      if (!get_landed()) {
        ROS_ERROR("[px4ctrl] Reject AUTO_TAKEOFF. land detector says that the "
                  "drone is not landed now!");
        break;
      }
      if (rc_is_received(now_time)) { // Check this only if RC is connected.
        ...
        if (!rc_data.is_hover_mode || !rc_data.is_command_mode ||
          ...
          ROS_ERROR(
              "[px4ctrl] Reject AUTO_TAKEOFF. If you have your RC connected, "
              "keep its switches at \"auto hover\" and \"command control\" "
              "states, and all sticks at the center, then takeoff again.");
          while (ros::ok()) {
    ...
    if (rc_dy_data
            .toggle_reboot) { // Try to reboot. EKF2 based PX4 FCU requires
  ...
  case CMD_CTRL: {
    ...
    if (takeoff_land_data.triggered && takeoff_land_data.takeoff_land_cmd ==
                                           quadrotor_msgs::TakeoffLand::LAND) {
      ROS_ERROR(
  ...
  case AUTO_TAKEOFF: {
    if ((now_time - takeoff_land.toggle_takeoff_land_time).toSec() <
        AutoTakeoffLand_t::MOTORS_SPEEDUP_TIME) { // Wait for several seconds to
                                                  // warn prople.
  ...
  case AUTO_LAND: {
    if (!rc_dy_data.is_hover_mode || !odom_is_received(now_time)) {
      ...
      if (print_once_flag) {
        ROS_INFO("\033[32m[px4ctrl] Wait for abount 10s to let the drone "
                 "arm.\033[32m");
  ...
  if (rotor_low_speed_during_land) { // used at the start of auto takeoff
    ...
    debug_msg =
        controller.calculateControl(des, odom_data, imu_data, u); //计算角度
