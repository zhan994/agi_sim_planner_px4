...
public:
  ...
  bool localTrajReachTarget() {
    return fabs(local_end_time_ - global_duration_) < 0.1;
  }
  ...
  void setLocalTraj(NonUniformBspline traj, double local_ts, double local_te,
                    double time_inc) {
  ...
  void getTrajByRadius(const double &start_t, const double &des_radius,
    ...
    Eigen::Vector3d first_pt =
        getPosition(start_t);           // first point of the segment
