...
bool FastPlannerManager::checkTrajCollision(double &distance) {
  ...
  Eigen::Vector3d cur_pt =
      local_data_.position_traj_.evaluateDeBoor(tm + t_now);
...
bool FastPlannerManager::kinodynamicReplan(Eigen::Vector3d start_pt,
  ...
  ctrl_pts = bspline_optimizers_[0]->BsplineOptimizeTraj(ctrl_pts, ts,
                                                         cost_function, 1, 1);
  ...
  cout << "[kino replan]: time: " << t_total << ", search: " << t_search
       << ", optimize: " << t_opt << ", adjust time:" << t_adjust << endl;
...
bool FastPlannerManager::topoReplan(bool collide) {
  ...
  Eigen::MatrixXd ctrl_pts =
      reparamLocalTraj(t_now, local_traj_dt, local_traj_duration);
  ...
  if (!collide) { // simply truncate the segment and do nothing
    ...
    global_data_.setLocalTraj(init_traj, t_now,
    ...
    if (colli_start.size() == 1 && colli_end.size() == 0) {
      ...
      global_data_.setLocalTraj(local_data_.position_traj_, t_now,
                                local_traj_duration + time_inc + t_now,
                                time_inc);
    }
  }
  updateTrajInfo();
...
void FastPlannerManager::refineTraj(NonUniformBspline &best_traj,
  ...
  ctrl_pts = bspline_optimizers_[0]->BsplineOptimizeTraj(ctrl_pts, dt,
                                                         cost_function, 1, 1);
  best_traj = NonUniformBspline(ctrl_pts, 3, dt);
  ROS_WARN_STREAM("[Refine]: cost "
...
void FastPlannerManager::optimizeTopoBspline(double start_t, double duration,
  ...
  Eigen::MatrixXd opt_ctrl_pts1 =
      bspline_optimizers_[traj_id]->BsplineOptimizeTraj(
...
Eigen::MatrixXd FastPlannerManager::reparamLocalTraj(double start_t, double &dt,
                                                     double &duration) {
  ...
  global_data_.getTrajByRadius(start_t, pp_.local_traj_len_, pp_.ctrl_pt_dist,
                               point_set, start_end_derivative, dt, duration);
...
Eigen::MatrixXd FastPlannerManager::reparamLocalTraj(double start_t,
                                                     double duration,
                                                     int seg_num, double &dt) {
  vector<Eigen::Vector3d> point_set;
  vector<Eigen::Vector3d> start_end_derivative;

  global_data_.getTrajByDuration(start_t, duration, seg_num, point_set,
                                 start_end_derivative, dt);
  plan_data_.local_start_end_derivative_ = start_end_derivative;
...
void FastPlannerManager::planYaw(const Eigen::Vector3d &start_yaw) {
  ...
  yaw =
      bspline_optimizers_[1]->BsplineOptimizeTraj(yaw, dt_yaw, cost_func, 1, 1);
