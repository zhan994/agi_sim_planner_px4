...
void KinoReplanFSM::init(ros::NodeHandle &nh) {
  ...
  odom_sub_ =
      nh.subscribe("/odom_world", 1, &KinoReplanFSM::odometryCallback, this);
...
void KinoReplanFSM::changeFSMExecState(FSM_EXEC_STATE new_state,
                                       string pos_call) {
  ...
  if (have_target_) {
    ...
    double dist =
        planner_manager_->pp_.dynamic_
            ? edt_env->evaluateCoarseEDT(
                  end_pt_, /* time to program start + */ info->duration_)
        ...
        for (double theta = -90; theta <= 270; theta += dtheta) {
          for (double nz = 1 * dz; nz >= -1 * dz; nz -= dz) {
            ...
            dist =
                planner_manager_->pp_.dynamic_
                    ? edt_env->evaluateCoarseEDT(
                          new_pt, /* time to program start+ */ info->duration_)
...
bool KinoReplanFSM::callKinodynamicReplan() {
  ...
  if (plan_success) {
    ...
    visualization_->drawGeometricPath(plan_data->kino_path_, 0.075,
                                      Eigen::Vector4d(1, 1, 0, 0.4));
    visualization_->drawBspline(info->position_traj_, 0.1,
                                Eigen::Vector4d(1.0, 0, 0.0, 1), true, 0.2,
